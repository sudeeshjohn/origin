...
function os::build::check_binaries() {
  ...
  if [[ -f "${OS_OUTPUT_BINPATH}/${platform}/openshift-node-config" ]]; then
    if [[ "$($duexe --apparent-size -m "${OS_OUTPUT_BINPATH}/${platform}/openshift-node-config" | cut -f 1)" -gt "100" ]]; then
  ...
  if [[ -f "${OS_OUTPUT_BINPATH}/${platform}/pod" ]]; then
    if [[ "$($duexe --apparent-size -m "${OS_OUTPUT_BINPATH}/${platform}/pod" | cut -f 1)" -gt "50" ]]; then
